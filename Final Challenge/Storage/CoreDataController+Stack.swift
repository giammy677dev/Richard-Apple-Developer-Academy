...
class CoreDataController {
    ...
    func addRoadmap(_ roadmap: WritableRoadmap) -> CDRoadmap? {
        ...
        newRoadmap.setValue(roadmap.getRoadmapUUID(), forKey: "uuid")

        newRoadmap.setValue(addUUID(roadmap.getRoadmapUUID()), forKey: "usedID")
    ...
    func addStep(_ step: Step, to roadmap: WritableRoadmap) -> CDStep? {
        ...
        let parentRoadmap = fetchCDRoadmap(uuid: roadmap.getRoadmapUUID())

        newStep.setValue(0, forKey: "arrayID")
        newStep.setValue(step.title, forKey: "title")
        newStep.setValue(step.getStepUUID(), forKey: "uuid")

        newStep.setValue(addUUID(step.getStepUUID()), forKey: "usedID")
    ...
    func addStep(_ step: Step, to roadmap: UUID) -> CDStep? {
        ...
        guard let entityStep = NSEntityDescription.entity(forEntityName: "CDStep", in: self.context) else { return nil }
        let newStep = CDStep(entity: entityStep, insertInto: self.context)
    ...
    func addNode(_ node: Node) -> CDNode? {
        ...
        newNode.setValue(node.getNodeUUID(), forKey: "uuid")
        newNode.setValue(addUUID(node.getNodeUUID()), forKey: "usedID")
    ...
    func unlinkNode( _ node: Node, from step: Step) {
        ...
        nodeToRemove = fetchCDNode(uuid: node.getNodeUUID())

        let step = fetchCDStep(uuid: step.getStepUUID())
    ...
    func deleteRoadmap(_ roadmap: WritableRoadmap) {
        guard isUUIDInUse(roadmap.getRoadmapUUID()), let roadmapToRemove = fetchCDRoadmap(uuid: roadmap.getRoadmapUUID()) else {
            return
        }
    ...
    func deleteRoadmap(_ roadmapID: UUID) {
        guard isUUIDInUse(roadmapID), let roadmapToRemove = fetchCDRoadmap(uuid: roadmapID) else {
            return
        }
    ...
    func deleteStep(_ step: Step) {
        guard isUUIDInUse(step.getStepUUID()), let stepToRemove = fetchCDStep(uuid: step.getStepUUID()) else {
            return
        }
    ...
    func deleteStep(_ stepID: UUID) {
        guard isUUIDInUse(stepID), let stepToRemove = fetchCDStep(uuid: stepID) else {
            return
        }
    ...
    func deleteNode(_ node: Node) {
        guard isUUIDInUse(node.getNodeUUID()), let nodeToRemove = fetchCDNode(uuid: node.getNodeUUID()) else {
            return
        }
    ...
    func deleteNode(_ nodeID: UUID) {
        guard isUUIDInUse(nodeID), let nodeToRemove = fetchCDNode(uuid: nodeID) else {
            return
        }
    ...
    func updateNode(_ node: Node) -> CDNode? {
        if let nodeToUpdate = fetchCDNode(uuid: node.getNodeUUID()) {
    ...
    func updateStep(_ step: Step, of roadmap: WritableRoadmap) -> CDStep? {
        ...
        if let stepToUpdate = fetchCDStep(uuid: step.getStepUUID()) {
    ...
    func updateStep(_ step: Step, of roadmap: UUID) -> CDStep? {
        let stepID = step.getStepUUID()
        if let stepToUpdate = fetchCDStep(uuid: stepID) {
            stepToUpdate.setValue(0, forKey: "arrayID")
            stepToUpdate.setValue(step.title, forKey: "title")
    ...
    func moveNode(_ node: Node, in step: Step, at index: Int) {
        let cdnode = fetchCDNode(uuid: node.getNodeUUID())
        let cdstep = fetchCDStep(uuid: step.getStepUUID())
    ...
    func moveStep(_ step: Step, in roadmap: WritableRoadmap, at index: Int) {
        let cdstep = fetchCDStep(uuid: step.getStepUUID())
        let cdroadmap = fetchCDRoadmap(uuid: roadmap.getRoadmapUUID())
    ...
    func nodeFromRecord(_ cdnode: CDNode) -> Node {
        let node = Node(url: URL(string: cdnode.url!)!,
                        title: cdnode.title!,
                        id: cdnode.uuid!,
                        parent: cdnode.parentsStep?.uuid ?? K.readingListStepID,
                        tags: cdnode.tags,
                        text: cdnode.extractedText!,
                        propExtracted: cdnode.isTextProperlyExtracted,
                        creationTime: cdnode.creationTimestamp! as Date,
