...
class DatabaseInterface {
    ...
    public func save(_ node: Node) {
        // CloudKit
        let recordID = CKRecordID(recordName: node.getNodeUUID().uuidString, zoneID: CKRecordZoneID(zoneName: K.CKRecordZoneIDs.privateRecordZoneName, ownerName: CKCurrentUserDefaultName))
    ...
    public func save(_ roadmap: WritableRoadmap) {
        ...
        // CloudKit
        let recordID = CKRecordID(recordName: roadmap.getRoadmapUUID().uuidString, zoneID: CKRecordZoneID(zoneName: K.CKRecordZoneIDs.privateRecordZoneName, ownerName: CKCurrentUserDefaultName))
        let completion: (([CKRecordID: CKRecord]?, Error?) -> Void) = { (recordsDict, error) in
            guard let recordsDictionary = recordsDict else { debugPrint("No records dictionary found."); return }
            if let ckError = error as? CKError {
                // Handle saving errors and retry
    ...
    public func save(_ step: Step) {
        // CloudKit
        let recordID = CKRecordID(recordName: step.getStepUUID().uuidString, zoneID: CKRecordZoneID(zoneName: K.CKRecordZoneIDs.privateRecordZoneName, ownerName: CKCurrentUserDefaultName))
        let completion: (([CKRecordID: CKRecord]?, Error?) -> Void) = { (recordsDict, error) in
            guard let recordsDictionary = recordsDict else { debugPrint("No records dictionary found."); return }
            if let ckError = error as? CKError {
                // Handle saving errors and retry
